...
/* A half-dead travel hotspot must not hold the voice sender forever. The
 * websocket client applies this timeout to its lock and each transport write. */
#define ESP_XIAOZHI_SEND_TIMEOUT_MS 3000
...
esp_err_t esp_xiaozhi_websocket_send_text(esp_xiaozhi_websocket_handle_t handle, const char *text)
{
    ...
    ESP_RETURN_ON_FALSE(text_len <= INT_MAX, ESP_ERR_INVALID_SIZE, TAG, "Text frame is too large");
    int sent = esp_websocket_client_send_text(websocket->websocket_client, text, (int)text_len,
                                               pdMS_TO_TICKS(ESP_XIAOZHI_SEND_TIMEOUT_MS));
...
esp_err_t esp_xiaozhi_websocket_send_binary(esp_xiaozhi_websocket_handle_t handle, const uint8_t *data, size_t data_len)
{
    ...
    ESP_RETURN_ON_FALSE(data_len <= INT_MAX, ESP_ERR_INVALID_SIZE, TAG, "Binary frame is too large");
    int sent = esp_websocket_client_send_bin(websocket->websocket_client, (const char *)data, (int)data_len,
                                              pdMS_TO_TICKS(ESP_XIAOZHI_SEND_TIMEOUT_MS));
